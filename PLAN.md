...
| C10 | mindfull | os títulos deviam ficar todos com a mesma margem | all titles should share the same margin | Standardised title/body left margins (consistent containers) **and** matched the Figma title style: all page titles, section headings and the footer/about headings changed from `font-medium` (500) to `font-normal` (400) Inter Tight to match Figma's lighter titles | ✅ |
